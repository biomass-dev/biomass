        ...
        for i, condition in enumerate(self.conditions):
            ...
            if sol is None:
                ...
                self.simulations[self.obs_names.index("pEGFR_au"), i] = np.log10(
                    x[C.offset_pEGFR_CelllineH322M] + x[C.scale_pEGFR_CelllineH322M]*(
                        2*sol.y[V.pEGFRd] + 2*sol.y[V.pEGFRi] + 2*sol.y[V.pEGFRi_ph] + sol.y[V.pErbB12]
                        + sol.y[V.pErbB12i] + sol.y[V.pErbB12i_ph] + sol.y[V.pErbB13] + sol.y[V.pErbB13i]
                        + sol.y[V.pErbB13i_ph] + sol.y[V.pMetEGFR] + sol.y[V.pMetEGFRi] + sol.y[V.pMetEGFRi_ph]
                    )
                )
                self.simulations[self.obs_names.index("pErbB2_au"), i] = np.log10(
                    x[C.offset_pErbB2_CelllineH322M] + x[C.scale_pErbB2_CelllineH322M]*(
                        sol.y[V.pErbB12] + sol.y[V.pErbB12i] + sol.y[V.pErbB12i_ph] + 2*sol.y[V.pErbB2] + 2*sol.y[V.pErbB2i]
                        + 2*sol.y[V.pErbB2i_ph] + sol.y[V.pErbB32] + sol.y[V.pErbB32i] + sol.y[V.pErbB32i_ph]
                    )
                )
                self.simulations[self.obs_names.index("pErbB3_au"), i] = np.log10(
                    x[C.offset_pErbB3_CelllineH322M] + x[C.scale_pErbB3_CelllineH322M]*(
                        sol.y[V.pErbB13] + sol.y[V.pErbB13i] + sol.y[V.pErbB13i_ph] + sol.y[V.pErbB32]
                        + sol.y[V.pErbB32i] + sol.y[V.pErbB32i_ph] + 2*sol.y[V.pErbB3d] + 2*sol.y[V.pErbB3i]
                        + 2*sol.y[V.pErbB3i_ph] + sol.y[V.pMetErbB3] + sol.y[V.pMetErbB3i] + sol.y[V.pMetErbB3i_ph]
                    )
                )
                self.simulations[self.obs_names.index("pERK_au"), i] = np.log10(
                    x[C.offset_pERK_CelllineH322M] + x[C.scale_pERK_CelllineH322M]*(
                        sol.y[V.pERK]
                    )
                )
